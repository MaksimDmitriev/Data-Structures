 ...
 * keys and values
 * https://github.com/lyair1/Red-Black-Tree-Example/blob/master/src/RBTree.java
 */
...
public class RBTree {
	...
	/**
	 * O(1) create a tree with a root
	 * 
	 * @param root
	 *            - RBNode object will be the root of the tree
	 */
	...
	/**
	 * O(1) create infinity node with Null children
	 * 
	...
	/**
	 * O(1) create infinity node with a left child (infinity node cant have
	 * right child)
	 * 
	 * @param leftchild
	 *            - the left child of the infinity node
	 * @return infinity node with a left child
	 */
	private RBNode createInfinityNode(RBNode leftchild) {
		// create infinity node with a key of the maximum integer available in
		// java.
	...
	/**
	 * O(1) create a null node
	 * 
	 * @param parent
	 *            - the parent of this null node
	 * @return the null node already connected to its parent
	 */
	private RBNode createNullNode(RBNode parent) {
		// create a null node. this node will have the minimum integer value
		// available in java as a key
	...
	/**
	 * O(log(n)) public String search(int k)
	 *
	...
	/**
	 * O(log(n)) search for node
	 * 
	 * @param k
	 *            - the key to look for
	 * @param node
	 *            - the node to start looking from
	 * @return if found return the node with the same key and if not found
	 *         returns the nearest node for the location the node was supposed
	 *         to be found. if the parent is the infinity node and not part of
	 *         the tree returns null
	 */
	private RBNode SearchNode(int k, RBNode node) {
		// if the node that was given is the sentinel recursively call the
		// function with the tree root
		if (isInfinityNode(node)) {
			return SearchNode(k, node.Left);
		}

		if (isNullNode(node)) {
			// the node is not exist in the tree. we will return null if the
			// tree is empty.
			if (isInfinityNode(node.Parent)) {
				return null;
			} else {
				// we could not find the node so we will return the parent of
				// the imaginary node if it was exist in the tree
				return node.Parent;
			}
		}
		...
		if (node.Key > k) {
			...
			// recursively call searchNode with the left child of the current
			// node
			return SearchNode(k, node.Left);
		}
		...
		if (node.Key < k) {
			...
			// recursively call searchNode with the right child of the current
			// node
			return SearchNode(k, node.Right);
		}

		// if the current node key is not bigger and not lower it equals to the
		// key we are looking for
		return node;
	}

	/**
	 * O(log(n)) public int insert(int k, String v)
	 *
	...
	public int insert(int k, String v) {
		...
		if (y == null) {
			...
			// calling the method that will check if the tree is valid and if
			// not fix it
			counter = fixUpTree(z);
		}
	...
	/**
	 * O(log(n)) keep the red and black rules. this method will be called when
	 * the rules were been compromised and needs to be fix.
	 * 
	 * @param z
	 *            - the node to start the fix from
		...
		while (!z.Parent.Black) {
			if (z.Parent == z.Parent.Parent.Left) {
				...
				if (!y.Black) {
					...
					if (z == z.Parent.Right) {
						// case 2: z is a right child and its uncle is red. need
						// to left rotate
						z = z.Parent;
						leftRotate(z);
					}
					// case 3: z is a left child and its uncle is red. need to
					// right rotate
					z.Parent.Black = true;
					z.Parent.Parent.Black = false;
				...
				if (!y.Black) {
					...
					if (z == z.Parent.Left) {
						// case 2: z is a left child and its uncle is red. need
						// to right rotate
						z = z.Parent;
						rightRotate(z);
					}
					// case 3: z is a right child and its uncle is red. need to
					// left rotate
					z.Parent.Black = true;
					z.Parent.Parent.Black = false;
	...
	/**
	 * O(1) put y instead of x
	 * 
	 * @param x
	 *            - the original child
	 * @param y
	 *            - the child after the change
	 */
	...
	/**
	 * O(1) rotate the tree left (according to the principles discussed in class
	 * and can be found in Book: Cormen - introduction to algorithms
	 * 
	 * @param x
	 *            - the node to rotate from
	 */
	...
	/**
	 * O(1) rotate the tree right (according to the principles discussed in
	 * class and can be found in Book: Cormen - introduction to algorithms
	 * 
	 * @param x
	 *            - the node to rotate from
	 */
	...
	/**
	 * O(log(n)) public int delete(int k)
	 *
	...
	/**
	 * O(log(n)) fix the red and black rules after deleting a node.
	 * 
	 * @param x
	 *            - the node to start the fixing from
	...
	/**
	 * O(log(n)) return the node with the minimal key in the tree
	 * 
	 * @param node
	 *            - the node to start looking from
	...
	/**
	 * O(1) check if the given node is an infinity node
	 * 
	 * @param node
	 *            - the node to check
	...
	/**
	 * O(1) check if the given node is a null node
	 * 
	 * @param node
	 *            - the node to check
	...
	/**
	 * O(log(n)) Returns the value of the item with the largest key in the tree,
	 * 
	 * @param node
	 *            the node to start looking from
	 ...
	 * 
	 * @param strArr
	 *            - array of string values (that can be parsed into integers)
	 ...
	 * @param key
	 *            - if true returns the string with keys elements else return
	 *            the string with value elements
	...
	private String ElementsToString(RBNode node, boolean key) {
		...
		if (!isNullNode(node.Right)) {
			// there is no lower elements return the current element + all the
			// elements that bigger
			return str + "," + ElementsToString(node.Right, key);
		}

		if (!isNullNode(node.Left)) {
			// there is no higher elements. return all the elements that are
			// lower than the current element
	...
	/**
	 * O(n) public String[] valuesToArray()
	 *
	...
	public class RBNode {
		...
		/**
		 * O(1) create node with null nodes children
		 * 
		 * @param value
